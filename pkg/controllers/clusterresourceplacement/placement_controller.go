...
import (
	...
	fleetplacementv1alpha1 "go.goms.io/fleet/apis/placement/v1alpha1"
...
func (r *Reconciler) handleUpdate(ctx context.Context, crp *fleetplacementv1alpha1.ClusterResourcePlacement) (ctrl.Result, error) {
	...
	if latestPolicySnapshot != nil &&
		...
		latestPolicySnapshot.Labels[fleetplacementv1alpha1.IsLatestSnapshotLabel] == strconv.FormatBool(true) {
		// set the latest label to false first to make sure there is only one or none active policy snapshot
		latestPolicySnapshot.Labels[fleetplacementv1alpha1.IsLatestSnapshotLabel] = strconv.FormatBool(false)
	...
	if latestPolicySnapshot == nil || string(latestPolicySnapshot.Spec.PolicyHash) != policyHash {
		...
		latestPolicySnapshot = &fleetplacementv1alpha1.ClusterPolicySnapshot{
			ObjectMeta: metav1.ObjectMeta{
				Name: fmt.Sprintf(fleetplacementv1alpha1.PolicySnapshotNameFmt, crp.Name, latestPolicySnapshotIndex),
				Labels: map[string]string{
					fleetplacementv1alpha1.CRPTrackingLabel:      crp.Name,
					fleetplacementv1alpha1.IsLatestSnapshotLabel: strconv.FormatBool(true),
					fleetplacementv1alpha1.PolicyIndexLabel:      strconv.Itoa(latestPolicySnapshotIndex),
				},
			},
			Spec: fleetplacementv1alpha1.PolicySnapshotSpec{
	...
	} else if latestPolicySnapshot.Labels[fleetplacementv1alpha1.IsLatestSnapshotLabel] != strconv.FormatBool(true) {
...
func (r *Reconciler) lookupLatestClusterPolicySnapshot(ctx context.Context, crp *fleetplacementv1alpha1.ClusterResourcePlacement) (*fleetplacementv1alpha1.ClusterPolicySnapshot, int, error) {
	snapshotList := &fleetplacementv1alpha1.ClusterPolicySnapshotList{}
	...
	if err := r.Client.List(ctx, snapshotList, client.MatchingLabels{fleetplacementv1alpha1.CRPTrackingLabel: crp.Name}); err != nil {
...
func parsePolicyIndexFromLabel(s *fleetplacementv1alpha1.ClusterPolicySnapshot) (int, error) {
	indexLabel := s.Labels[fleetplacementv1alpha1.PolicyIndexLabel]
...
func generatePolicyHash(policy *fleetplacementv1alpha1.PlacementPolicy) (string, error) {
