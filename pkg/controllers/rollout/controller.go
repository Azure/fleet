...
func isBindingReady(binding *fleetv1beta1.ClusterResourceBinding, readyTimeCutOff time.Time) (time.Duration, bool) {
	...
	if condition.IsConditionStatusFalse(diffReportCondition, binding.GetGeneration()) {
		// we wait for the reportDiff condition to be change as we monitor it.
		return -1, false
	}
...
func handleResourceBindingUpdated(objectOld, objectNew client.Object, q workqueue.RateLimitingInterface) {
	...
	if oldBinding.GetResourceVersion() != newBinding.GetResourceVersion() {
