...
func (r *Reconciler) determinePolicySnapshot(
	...
	if !foundIndex || len(policyIndex) == 0 {
		...
		return nil, -1, fmt.Errorf("%w: %s", errValidationFailed, noIndexErr.Error())
	}
	...
	if policySnapshotSpec.Policy != nil {
		if policySnapshotSpec.Policy.PlacementType == placementv1beta1.PickNPlacementType {
			...
			if err != nil {
				...
				// no more retries here.
				return nil, -1, fmt.Errorf("%w, %s", errValidationFailed, annErr.Error())
			}
			clusterCount = count
	...
	if !condition.IsConditionStatusTrue(latestPolicySnapshot.GetCondition(string(placementv1beta1.PolicySnapshotScheduled)), latestPolicySnapshot.GetGeneration()) {
		...
		return nil, -1, fmt.Errorf("%w: %s", errValidationFailed, scheduleErr.Error())
	}
...
func (r *Reconciler) getResourceSnapshotObjs(ctx context.Context, placement placementv1beta1.PlacementObj, updateRun placementv1beta1.UpdateRunObj) ([]placementv1beta1.ResourceSnapshotObj, error) {
	...
	if updateRunSpec.ResourceSnapshotIndex != "" {
		...
		if err != nil || snapshotIndex < 0 {
			...
			// no more retries here.
			return nil, fmt.Errorf("%w: %s", errValidationFailed, userErr.Error())
		}
		...
		if len(resourceSnapshotObjs) == 0 {
			...
			// no more retries here.
			return resourceSnapshotObjs, fmt.Errorf("%w: %s", errValidationFailed, userErr.Error())
		}
