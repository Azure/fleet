...
type pluginState struct {
	...
	preferredAffinityTerms *framework.PreferredAffinityTerms
}
...
func preparePluginState(policy *fleetv1beta1.ClusterSchedulingPolicySnapshot) (*pluginState, error) {
	...
	var requiredTerms []framework.AffinityTerm
	var err error
	if clusterAffinity.RequiredDuringSchedulingIgnoredDuringExecution != nil &&
		...
		requiredTerms, err = framework.NewAffinityTerms(clusterAffinity.RequiredDuringSchedulingIgnoredDuringExecution.ClusterSelectorTerms)
		if err != nil {
			return nil, err
		}
	}
	var preferredTerms *framework.PreferredAffinityTerms
	if len(clusterAffinity.PreferredDuringSchedulingIgnoredDuringExecution) > 0 {
		preferredTerms, err = framework.NewPreferredAffinityTerms(clusterAffinity.PreferredDuringSchedulingIgnoredDuringExecution)
		if err != nil {
			return nil, err
		}
	}
