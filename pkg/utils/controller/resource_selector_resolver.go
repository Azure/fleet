...
type ResourceSelectorResolver struct {
	...
	// RestMapper is the rest mapper used to convert between gvk and gvr on known resources.
	RestMapper meta.RESTMapper

	// EnableWorkload indicates whether workload resources are allowed to be selected.
	EnableWorkload bool
}
...
func (rs *ResourceSelectorResolver) SelectResourcesForPlacement(placementObj placementv1beta1.PlacementObj) (int, []placementv1beta1.ResourceContent, []placementv1beta1.ResourceIdentifier, error) {
	envelopeObjCount := 0
	selectedObjects, err := rs.gatherSelectedResource(types.NamespacedName{
	...
	resources := make([]placementv1beta1.ResourceContent, len(selectedObjects))
	resourcesIDs := make([]placementv1beta1.ResourceIdentifier, len(selectedObjects))
...
func generateResourceContent(object *unstructured.Unstructured) (*placementv1beta1.ResourceContent, error) {
	rawContent, err := generateRawContent(object)
	if err != nil {
		return nil, NewUnexpectedBehaviorError(err)
	}
	return &placementv1beta1.ResourceContent{
...
func generateRawContent(object *unstructured.Unstructured) ([]byte, error) {
	...
	if annots != nil {
		...
		// Remove the revision annotation set by deployment
...
func (rs *ResourceSelectorResolver) fetchNamespaceResources(selector placementv1beta1.ResourceSelectorTerm, placementName string) ([]runtime.Object, error) {
	klog.V(2).InfoS("start to fetch the namespace resources by the selector", "selector", selector)
	var resources []runtime.Object

	if len(selector.Name) != 0 {
		// just a single namespace
		objs, err := rs.fetchAllResourcesInOneNamespace(selector.Name, placementName)
		if err != nil {
	...
	namespaces, err := rs.InformerManager.Lister(utils.NamespaceGVR).List(labelSelector)
	if err != nil {
	...
	for _, namespace := range namespaces {
		...
		if err != nil {
			return nil, NewUnexpectedBehaviorError(fmt.Errorf("cannot get the name of a namespace object: %w", err))
		}
		objs, err := rs.fetchAllResourcesInOneNamespace(ns.GetName(), placementName)
		if err != nil {
...
func (rs *ResourceSelectorResolver) fetchAllResourcesInOneNamespace(namespaceName string, placeName string) ([]runtime.Object, error) {
	...
	obj, err := rs.InformerManager.Lister(utils.NamespaceGVR).Get(namespaceName)
	if err != nil {
		klog.ErrorS(err, "cannot get the namespace", "namespace", namespaceName)
		return nil, NewAPIServerError(true, client.IgnoreNotFound(err))
	}
	...
	for _, gvr := range trackedResource {
		...
		if !rs.InformerManager.IsInformerSynced(gvr) {
			return nil, NewExpectedBehaviorError(fmt.Errorf("informer cache for %+v is not synced yet", gvr))
		}
		...
		for _, obj := range objs {
			shouldInclude, err := rs.ShouldPropagateObj(namespaceName, placeName, obj)
			if err != nil {
				return nil, err
			}
			if shouldInclude {
...
func (rs *ResourceSelectorResolver) fetchResources(selector placementv1beta1.ResourceSelectorTerm, placementKey types.NamespacedName) ([]runtime.Object, error) {
	klog.V(2).InfoS("Start to fetch resources by the selector", "selector", selector, "placement", placementKey)
	gk := schema.GroupKind{
		Group: selector.Group,
		Kind:  selector.Kind,
	}
	restMapping, err := rs.RestMapper.RESTMapping(gk, selector.Version)
	if err != nil {
		return nil, NewUserError(fmt.Errorf("invalid placement %s, failed to get GVR of the selector: %w", placementKey, err))
	}
	...
	isNamespacedResource := !rs.InformerManager.IsClusterScopedResources(gvk)
	...
	if !rs.InformerManager.IsInformerSynced(gvr) {
		...
		return nil, NewExpectedBehaviorError(err)
	}
	...
	if len(selector.Name) != 0 {
		...
		shouldInclude, err := rs.ShouldPropagateObj(placementKey.Namespace, placementKey.Name, obj)
		if err != nil {
			return nil, err
		}
		if shouldInclude {
	...
	for i := 0; i < len(objects); i++ {
		shouldInclude, err := rs.ShouldPropagateObj(placementKey.Namespace, placementKey.Name, objects[i])
		if err != nil {
			return nil, err
		}
		if shouldInclude {
...
func (rs *ResourceSelectorResolver) ShouldPropagateObj(namespace, placementName string, obj runtime.Object) (bool, error) {
	...
	shouldInclude, err := ShouldPropagateObj(rs.InformerManager, uObj, rs.EnableWorkload)
	if err != nil {
...
func ShouldPropagateObj(informerManager informer.Manager, uObj *unstructured.Unstructured, enableWorkload bool) (bool, error) {
	...
	case appv1.SchemeGroupVersion.WithKind(utils.ReplicaSetKind):
	...
	case corev1.SchemeGroupVersion.WithKind(utils.ConfigMapKind):
	...
	case corev1.SchemeGroupVersion.WithKind("PersistentVolumeClaim"):
		// Skip PersistentVolumeClaims by default to avoid conflicts with the PVCs created by statefulset
