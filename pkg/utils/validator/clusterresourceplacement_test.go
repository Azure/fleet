...
func TestIsFinalizerRemoved(t *testing.T) {
	tests := map[string]struct {
		oldFinalizers     []string
		currentFinalizers []string
		want              bool
	}{
		"no update to finalizer": {
			oldFinalizers:     []string{"finalizer1", "finalizer2"},
			currentFinalizers: []string{"finalizer1", "finalizer2"},
			want:              false,
		},
		"finalizer removed, length of current finalizers greater than 0": {
			oldFinalizers:     []string{"finalizer1", "finalizer2"},
			currentFinalizers: []string{"finalizer1"},
			want:              true,
		},
		"finalizer removed, length of current finalizers equal to 0": {
			oldFinalizers:     []string{"finalizer1", "finalizer2"},
			currentFinalizers: []string{},
			want:              true,
		},
		"finalizer added": {
			oldFinalizers:     []string{"finalizer1", "finalizer2"},
			currentFinalizers: []string{"finalizer1", "finalizer2", "finalizer3"},
			want:              false,
		},
		"finalizer updated": {
			oldFinalizers:     []string{"finalizer1", "finalizer2"},
			currentFinalizers: []string{"finalizer1", "finalizer3"},
			want:              false,
		},
	}
	for testName, testCase := range tests {
		t.Run(testName, func(t *testing.T) {
			if got := IsFinalizerRemoved(testCase.oldFinalizers, testCase.currentFinalizers); got != testCase.want {
				t.Errorf("IsFinalizerRemoved() got = %v, want = %v", got, testCase.want)
			}
		})
	}
}
