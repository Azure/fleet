...
func ValidateUserForResource(whiteListedUsers []string, userInfo authenticationv1.UserInfo, resKind, resName string) admission.Response {
	if !IsMasterGroupUserOrWhiteListedUser(whiteListedUsers, userInfo) {
		return admission.Denied(fmt.Sprintf("failed to validate user: %s in groups: %v to modify fleet %s: %s", userInfo.Username, userInfo.Groups, resKind, resName))
	}
	...
	return admission.Allowed(fmt.Sprintf("user: %s in groups: %v is allowed to modify fleet %s: %s", userInfo.Username, userInfo.Groups, resKind, resName))
}
