...
var _ = Describe("Fleet's Reserved Namespaced Resources Handler webhook tests", func() {
	Context("fleet guard rail e2e for role in rbac/v1 api group", func() {
		...
		It("should deny CREATE operation on role for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &roleGVK, "", types.NamespacedName{Name: r.Name, Namespace: r.Namespace})))
		})
		...
		It("should deny UPDATE operation on role for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &roleGVK, "", types.NamespacedName{Name: r.Name, Namespace: r.Namespace})))
				return nil
		...
		It("should deny DELETE operation on role for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &roleGVK, "", types.NamespacedName{Name: r.Name, Namespace: r.Namespace})))
		})
	...
	Context("fleet guard rail e2e for role binding in rbac/v1 api group", func() {
		...
		It("should deny CREATE operation on role binding for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &roleBindingGVK, "", types.NamespacedName{Name: rb.Name, Namespace: rb.Namespace})))
		})
		...
		It("should deny UPDATE operation on role binding for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &roleBindingGVK, "", types.NamespacedName{Name: rb.Name, Namespace: rb.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE operation on role binding for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &roleBindingGVK, "", types.NamespacedName{Name: rb.Name, Namespace: rb.Namespace})))
		})
	...
	Context("fleet guard rail e2e for pod in core/v1 api group", func() {
		...
		It("should deny CREATE pod operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &podGVK, "", types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace})))
		})
		...
		It("should deny UPDATE pod operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &podGVK, "", types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE pod operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &podGVK, "", types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace})))
		})
	...
	Context("fleet guard rail e2e for deployment in apps/v1 api group", func() {
		...
		It("should deny CREATE deployment operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &deploymentGVK, "", types.NamespacedName{Name: d.Name, Namespace: d.Namespace})))
		})
		...
		It("should deny UPDATE deployment operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &deploymentGVK, "", types.NamespacedName{Name: d.Name, Namespace: d.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE deployment operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &deploymentGVK, "", types.NamespacedName{Name: d.Name, Namespace: d.Namespace})))
		})
	...
	Context("fleet guard rail e2e for job in batch/v1 api group", func() {
		...
		It("should deny CREATE job operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &jobGVK, "", types.NamespacedName{Name: j.Name, Namespace: j.Namespace})))
		})
		...
		It("should deny UPDATE job operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &jobGVK, "", types.NamespacedName{Name: j.Name, Namespace: j.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE job operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &jobGVK, "", types.NamespacedName{Name: j.Name, Namespace: j.Namespace})))
		})
	...
	Context("fleet guard rail e2e for horizontal pod auto scaler", func() {
		...
		It("should deny CREATE horizontal pod auto scaler operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &hpaGVK, "", types.NamespacedName{Name: hpa.Name, Namespace: hpa.Namespace})))
		})
		...
		It("should deny UPDATE horizontal pod auto scaler operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &hpaGVK, "", types.NamespacedName{Name: hpa.Name, Namespace: hpa.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE horizontal pod auto scaler operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &hpaGVK, "", types.NamespacedName{Name: hpa.Name, Namespace: hpa.Namespace})))
		})
	...
	Context("fleet guard rail e2e for lease", func() {
		...
		It("should deny CREATE horizontal lease operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &leaseGVK, "", types.NamespacedName{Name: l.Name, Namespace: l.Namespace})))
		})
		...
		It("should deny UPDATE lease operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &leaseGVK, "", types.NamespacedName{Name: l.Name, Namespace: l.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE lease operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &leaseGVK, "", types.NamespacedName{Name: l.Name, Namespace: l.Namespace})))
		})
	...
	Context("fleet guard rail e2e for end point slices", func() {
		...
		It("should deny CREATE endpoint slice for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &epsGVK, "", types.NamespacedName{Name: e.Name, Namespace: e.Namespace})))
		})
		...
		It("should deny UPDATE end point slice operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &epsGVK, "", types.NamespacedName{Name: e.Name, Namespace: e.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE endpoint slice operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &epsGVK, "", types.NamespacedName{Name: e.Name, Namespace: e.Namespace})))
		})
	...
	Context("fleet guard rail e2e for ingress", func() {
		...
		It("should deny CREATE ingress for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &ingressGVK, "", types.NamespacedName{Name: i.Name, Namespace: i.Namespace})))
		})
		...
		It("should deny UPDATE ingress operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &ingressGVK, "", types.NamespacedName{Name: i.Name, Namespace: i.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE ingress operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &ingressGVK, "", types.NamespacedName{Name: i.Name, Namespace: i.Namespace})))
		})
	...
	Context("fleet guard rail e2e for pod disruption budgets", func() {
		...
		It("should deny CREATE pod disruption budget for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &pdbGVK, "", types.NamespacedName{Name: pdb.Name, Namespace: pdb.Namespace})))
		})
		...
		It("should deny UPDATE pod disruption budget operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &pdbGVK, "", types.NamespacedName{Name: pdb.Name, Namespace: pdb.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE pod disruption budget operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &pdbGVK, "", types.NamespacedName{Name: pdb.Name, Namespace: pdb.Namespace})))
		})
	...
	Context("fleet guard rail e2e for CSI storage capacity", func() {
		...
		It("should deny CREATE CSI storage capacity for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Create, &csiGVK, "", types.NamespacedName{Name: csc.Name, Namespace: csc.Namespace})))
		})
		...
		It("should deny UPDATE CSI storage capacity operation for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Update, &csiGVK, "", types.NamespacedName{Name: csc.Name, Namespace: csc.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny DELETE CSI storage capacity operation for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, admissionv1.Delete, &csiGVK, "", types.NamespacedName{Name: csc.Name, Namespace: csc.Namespace})))
		})
