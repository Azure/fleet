...
var _ = Describe("Fleet's CRD Resource Handler webhook tests", func() {
	Context("CRD validation webhook", func() {
		It("should deny CREATE operation on Fleet CRD for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "CustomResourceDefinition", "", types.NamespacedName{Name: crd.Name})))
		})
		...
		It("should deny UPDATE operation on Fleet CRD for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "CustomResourceDefinition", "", types.NamespacedName{Name: crd.Name})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
...
var _ = Describe("Fleet's Custom Resource Handler webhook tests", func() {
	...
	Context("fleet guard rail tests for MC, IMC", func() {
		It("should deny CREATE operation on member cluster CR for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "MemberCluster", "", types.NamespacedName{Name: mc.Name})))
		})
		...
		It("should deny UPDATE operation on member cluster CR for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "MemberCluster", "", types.NamespacedName{Name: mc.Name})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny member cluster CR status update for user not in system:master group or a whitelisted user", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "MemberCluster", "status", types.NamespacedName{Name: mc.Name})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny CREATE operation on internal member cluster CR for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "InternalMemberCluster", "", types.NamespacedName{Name: imc.Name, Namespace: imc.Namespace})))
		})
		...
		It("should deny UPDATE operation on internal member cluster CR for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "InternalMemberCluster", "", types.NamespacedName{Name: imc.Name, Namespace: imc.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
		...
		It("should deny UPDATE operation on internal member cluster CR status for user in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, "kubernetes-admin", []string{"system:masters", "system:authenticated"}, "InternalMemberCluster", "status", types.NamespacedName{Name: imc.Name, Namespace: imc.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
...
var _ = Describe("Fleet's Work Resource Handler webhook tests", func() {
	Context("fleet guard rail for work resource, no need to get MC", func() {
		...
		It("should deny CREATE operation on work CR for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Work", "", types.NamespacedName{Name: w.Name, Namespace: w.Namespace})))
		})
		...
		It("should deny UPDATE operation on work CR for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Work", "", types.NamespacedName{Name: w.Name, Namespace: w.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
	...
	Context("fleet guard rail for work resource, need to get MC", func() {
		...
		It("should deny UPDATE operation on work CR status for user in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, "kubernetes-admin", []string{"system:masters", "system:authenticated"}, "Work", "status", types.NamespacedName{Name: w.Name, Namespace: w.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
	})
...
var _ = Describe("Fleet's Event Resource Handler webhook tests", func() {
	Context("fleet guard rail for event resource, no need to get MC", func() {
		...
		It("should deny CREATE operation on event resource for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Event", "", types.NamespacedName{Name: e.Name, Namespace: e.Namespace})))
		})
		...
		It("should deny UPDATE operation on event for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Event", "", types.NamespacedName{Name: e.Name, Namespace: e.Namespace})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
...
var _ = Describe("Fleet's Reserved Namespace Handler webhook tests", func() {
	Context("deny requests to modify namespace with fleet/kube prefix", func() {
		It("should deny CREATE operation on namespace with fleet prefix for user not in system:masters group", func() {
			...
			Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Namespace", "", types.NamespacedName{Name: ns.Name})))
		})
		...
		It("should deny UPDATE operation on namespace with fleet prefix for user not in system:masters group", func() {
			Eventually(func(g Gomega) error {
				...
				g.Expect(string(statusErr.Status().Reason)).Should(Equal(fmt.Sprintf(resourceDeniedFormat, testUser, testGroups, "Namespace", "", types.NamespacedName{Name: ns.Name})))
				return nil
			}, testutils.PollTimeout, testutils.PollInterval).Should(Succeed())
		})
